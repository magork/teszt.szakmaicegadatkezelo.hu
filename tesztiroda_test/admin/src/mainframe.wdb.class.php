...
//echo $sql;
		...
		if( $total > $perlap )
		{
			...
			{
				echo '<a href="index.php?do=comp_list&cat='.$cat.'&cat_name='.$cat_name.'&parent='.$parent.'&lap='. $pager['prev'] .'">Előző</a> <img src="images/sep.jpg" align="absmiddle" />';
			}
			...
			foreach ( $pager['tolok'] as $tol )
			{
					...
					if( $tol['selected'] )
					{
						echo'<a href="#" class="active"> ' . $tol['szam'] . ' </a>';
			...
			{
						echo'<img src="images/sep.jpg" align="absmiddle" /> <a href="index.php?do=comp_list&cat='.$cat.'&cat_name='.$cat_name.'&parent='.$parent.'&lap=' .  $pager['next'] . '">Következő</a>';
			}
		}
		...
		foreach( $eredmenyek as $res ){
				echo"<tr class='row' valign='top'>
								...
								<td valign='middle'>".$res[6]."</td>
								<td valign='middle'>".$res[5]."</td>
								<td valign='middle'>".$res[3]."</td> 
					 </tr>";
		}
	...
	function comp_show( $id, $cname ){
		...
		echo'<div id="table_wrapper">
         ...
         <div class="" style="position: relative; margin-left: 25px;">
            ...
            <table>
              <tr>
                <td align="left" valign="top" width="250"><strong>Cím:</strong> </td>
            ...
            <div class="text_block">
                   <strong>Tevékenységek</strong><br>
                   '.$data->advcat.'
            </div>
            <div class="text_block">
                    <strong>Cégbemutató</strong><br>
                    '.$data->description.'
            </div>
            <div class="text_block">
            ...
            <div class="text_block">
                    <strong>Telephelyek</strong><br>
                    '.$data->places.'
            </div>

            <div class="text_block">
                    <strong>Minősítés</strong><br>
                    '.$data->iso.'
            </div>
            <div class="text_block">
                    <strong>M&aacute;rk&aacute;k</strong><br>
                    '.$data->brands.'
            </div>';
...
//echo $sql;
		...
		echo "<div id='table_wrapper'><table cellpadding='0' cellspacing='0' id='lister' width='100%'>
		<thead> <th>Cég neve</th><th>Telefon</th><th>Város</th><th>E-mail</th> </thead>";
		...
		foreach( $eredmenyek as $res ){
				echo"<tr class='row' valign='middle'>
								<td valign='middle'><strong><a href='?do=comp_show&cid=".$res[0]."&cname=".$res[2]."'>".$res[2]."</a></strong></td>
								<td valign='middle'>".$res[6]."</td>
								<td valign='middle'>".$res[5]."</td>
								<td valign='middle'>".$res[3]."</td> 
					 </tr>";
	...
	function init(){
		...
		foreach ( $cats as $sor )
		{
				if( $sor->parent == 0 ) 
				{
					...
					echo "<u><a href='index.php?do=comp_list&cat=".$sor->id."&cat_name=".$sor->category."&parent=true'>".$sor->category."</a></u>\n";
					...
					$actual = $sor->id;
						...
						foreach ( $cats as $sorok )
						{
							...
							{
							  $even = ( $even == '' ) ? "class='odd'" : '';
							  echo "<a href='index.php?do=comp_list&cat=".$sorok->id."&cat_name=".$sorok->category."' ".$even."><span>&raquo;</span> ".$sorok->category."</a>\n ";
							}
						}
					echo"<br />\n";
