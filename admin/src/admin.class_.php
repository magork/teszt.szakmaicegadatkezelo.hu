...
class admin{
	...
	********************** */
	...
	function main_category_list($id = 0)
	{
		...
		$leker = $db->get_results("SELECT * FROM categories WHERE parent = '0'");
			...
			foreach( $leker as $sor ){
				...
				echo"<li><a href='#' onclick=\"deleteCat('".$sor->id."', '".$sor->category."')\"><img src='img/delete.gif' align='absmiddle'></a><a onclick='toggle(\"c".$sor->id."\");toggle(\"b".$sor->id."\")'> <img src='img/edit.gif' align='absmiddle' style='cursor: pointer;'></a> <a href='index.php?do=category_list&id=".$sor->id."' class='cat'>".$sor->category.$insert."</a><br><input type='text' name='".$sor->id."' value='".$sor->category."' style='display: none; margin-left:30px;' id='c".$sor->id."'><input type='button' value='Ok' onclick='modify( ".$sor->id." )' style='display: none;' id='b".$sor->id."'></li>\n";
	...
	function sub_category_list($id = 0)
	{
		...
		if( $leker != NULL ){
			...
			foreach($leker as $sor){
					...
					// A köv. soron be van linkelve a kategória:
					//echo"<li><a href='?do=category_delete&id=".$sor->id."'><img src='img/delete.gif' align='absmiddle'></a><a onclick='toggle(\"c".$sor->id."\");toggle(\"b".$sor->id."\")'><img src='img/edit.png' align='absmiddle'></a> <a href='index.php?do=category_list&id=".$sor->id."' class='cat'>".$sor->category."</a><br><input type='text' name='".$sor->id."' value='".$sor->category."' style='display: none; margin-left:30px;' id='c".$sor->id."'><input type='button' value='Ok' onclick='modify( ".$sor->id." )' style='display: none;' id='b".$sor->id."'></li>\n";
			}
		}
		else
		{
	...
	function category_edit( $str, $id = 0 )
	{
		global $db;
		$leker = $db->query("UPDATE categories SET category = '".$str."' WHERE id = '".$id."'");
		//
		$parent = $db->get_var("SELECT parent FROM categories WHERE id='".$id."'");
		if( $parent == 0 ) $parent = '';
		echo"<script> document.location = 'index.php?do=category_list&id=".$parent."' </script>";
...
STUFF;
	...
	************************ */
	...
	function comp_list( $search = '' )
	{
		...
		function get_cats( $col1, $col2, $col3, $col4 ,$col5, $col6, $col7, $col8, $col9, $col10, $col11, $col12, $col13, $col14, $col15, $col16, $col17 ){
			global $db;
			global $regions;
...
*/
			...
			if ( strlen( $col8 ) > 23 ) $col8 = substr( $col8, 0, 23 ) . '...';

			$col5 = $regions[ $col5 ];

			$col13 = date("Y/m/d H:i",$col13);
		}
		...
		if( $_SESSION['who'] == 'S' )
		{
			$ezr->results_row = "<tr class='row' valign='top'> 
									<td><strong>COL3</strong><br /><br /> <strong>Rögzítve</strong>:<br /> COL13<br /><br /><em><strong>Minősítés:</strong></em><br>COL17</td>
									<td><em><strong>Web:</strong></em><br /><a href='COL8' target='new'>COL8</a><br /> <em><strong>E-mail:</strong></em><br /> <a href='mailto:COL4'>COL4</a> <br /><em><strong>Telefon:</strong></em><br /><strong>COL7</strong> <em><strong><br />Fax:</strong></em><br /><strong>COL16</strong></td>
									<td><strong>COL6</strong> <br> COL5</td>
									<td><strong>Márkák:</strong><br>COL17</td> 
									<td>COL2 <br /><a onclick='toggle(\"xCOL1\")' style='cursor: pointer;'><img src='img/add.png' align='absmiddle'>Telephely</a> <span id='xCOL1' style='display:none;'><hr> COL11</span></td>
...
*/
	...
	********************** */
	...
	{
		global $db;
		echo'<form action="index.php?do=comp_parse" method="post" enctype="application/x-www-form-urlencoded" onsubmit="return checkBoxes()">
	...
	********************** */
	...
	function comp_parse( $cname, $address, $places, $email, $region, $city, $tel, $url, $brands, $description, $cats, $advcat, $fax, $iso )
	{
		global $db;
		...
		if( !empty( $cats ) ){
			$leker = $db->query("INSERT INTO companies ( id, address, cname, email, region, city, tel, url, brands, description, places, pass, ts, advcat, fax, iso ) VALUES ( '', '$address', '$cname', '$email', '$region', '$city', '$tel', '$url', '$brands', '$description', '$places', '$pass', UNIX_TIMESTAMP(), '$advcat', '$fax', '$iso' ) ");
	...
	********************** */
	...
	function comp_edit( $id )
	{
		...
		echo'<form action="index.php?do=comp_parse_edit" method="post" enctype="application/x-www-form-urlencoded">
				...
				<td width="400" valign="top" bgcolor="#ffffff">
					...
					<input type="hidden" name="id" value="'.$id.'" />
						<table width="400" border="0" cellspacing="3" cellpadding="1" bgcolor="">
						  ...
						  <tr>
							<td align="right" valign="top">Egyéb tevékenységek</td>
							<td><textarea name="advcat" cols="30" rows="4" id="description">'.$data->advcat.'</textarea></td>
						  </tr>
						  <tr>
							<td align="right" valign="middle">&nbsp;</td>
	...
	***************************** */
	...
	function comp_parse_edit( $id, $cname, $address, $places, $email, $region, $city, $tel, $url, $brands, $description, $cats, $page, $advcat, $fax, $iso )
	{
		global $db;
	...
	*/
	...
	function b2b_pass( $q )
	{
		...
		$ezr->results_row = "<tr class='row' valign='top'> 
								...
								<td align='center'><img src='img/COL14.gif'></td> 
							</tr>";
		$ezr->text_count = "NUMBER cég az adatbázisban";
	...
	function b2b_sendmail( $id )
	{
		...
		$message = "<h3>Tisztelt ".$data->cname.",</h3>Az alábbi jelszóval be tud lépni B2B ajánlatküldő rendszerébe:<br> A jelszó: ".$data->pass." <br> Az alábbi linken a beléptető oldalra kerül.<br><br> <a href='http://www.uniostudakozo.hu'>Tovább az Unióstudakozó oldalára.</a>";
		...
		$mail->CharSet  = "utf-8";
		$mail->From     = 'noreply@uniostudakozo.hu';
		$mail->FromName = "Uniostudakozo";
		$mail->Mailer   = "mail";
	...
	***************************** */
	...
	function b2b_list( $type )
	{
		...
		$ezr->results_row = "<tr class='row'>
							 ...
							 <tr class='row' valign='top'> 
								 ...
								 <p align='right'><a href='javascript:deleteOffer( \"COL1\" )'><font size='3'><img src='img/delete.gif' title='Törlés' align='absmiddle'> TÖRLÉS </font></p>
								 </td>
							 </tr>";
		...
		if( $type == 'over' ){
			...
			$fortnight = mktime(date("H", time()), date("i", time()), 0, date("m", time()), date("d", time())+14, date("Y", time()));
			$ezr->query_mysql("SELECT * FROM offers WHERE ts + '1209600' < UNIX_TIMESTAMP() AND active = '1'");
	...
	function b2b_activate()
	{
		...
		$ezr->query_mysql("SELECT * FROM offers WHERE ts + '1209600' > UNIX_TIMESTAMP() AND active = 0");
